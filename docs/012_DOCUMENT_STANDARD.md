# DOCUMENT STANDARD

## Every document has metadata. No exceptions.

---

## Required fields

| Field | Description | Example |
|-------|-------------|---------|
| **Version** | Semantic version of this document | 1.0 |
| **Status** | Draft, Review, Approved, Superseded | Approved |
| **Owner** | Team or person responsible | Product Team |
| **Review Date** | Next scheduled review | 2026-10-08 |
| **Dependencies** | Documents this depends on | DOCUMENT 002 |
| **Related Documents** | Documents related to this topic | DOCUMENT 005, DOCUMENT 006 |
| **Change Log** | History of significant changes | See below |
| **Acceptance Criteria** | How to verify this document is complete | See below |

## Change log format

```
v1.0 — 2026-07-08 — Initial version
v1.1 — YYYY-MM-DD — Summary of change
```

## Acceptance criteria format

```
- [ ] Document is internally consistent
- [ ] All claims have evidence or references
- [ ] All dependencies are documented
- [ ] Owner has approved
- [ ] Next review date is set
```

## Placement

Metadata block appears immediately after the document title, before any content.
