// STRIDE SCENE A — the blue "mesh gradient" backdrop.
//
// A static fullscreen fragment shader: soft-edged gaussian dark blobs overlapping
// into one near-black mass over a blue field, with a bright bottom-left glow and a
// softer top-right glow. It renders ONCE (and again on resize / colour change) —
// there is no render loop, so it costs nothing while the page scrolls.
//
// PORTED FROM three r0.143 TO THE PROJECT'S r0.185. One API change: `WebGL1Renderer`
// was removed in r163, so this uses `WebGLRenderer`. The shader is byte-identical to
// the source — the blob positions, radii and weights are baked artwork, not tunables.
...
export function createGradientBackground(
  ...
  return {
    ...
    dispose() {
      ...
      renderer.dispose()
      // Release the context so a remount (HMR, route re-entry) can attach a fresh
