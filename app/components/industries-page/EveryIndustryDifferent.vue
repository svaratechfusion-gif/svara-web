...
// EVERY INDUSTRY IS DIFFERENT — visual transformation:
// Operational Challenges → Connected Intelligence → Automation →
// Optimization → Business Outcomes. Progressive scroll reveal.
import { ref, onMounted, onBeforeUnmount } from 'vue'
import { ScrollTrigger } from '~~/lib/gsap'
import { TRANSFORM_STORY } from '~/utils/industries-page'
...
onMounted(() => {
  ...
  st = ScrollTrigger.create({ trigger: root.value!, start: 'top 72%', end: 'bottom 75%', scrub: true, onUpdate: (self) => { active.value = Math.floor(self.progress * TRANSFORM_STORY.length) } })
})
...
<template>
  <section ref="root" class="ei">
    <div class="ei__head">
      <p class="ei__eyebrow">Every Industry Is Different</p>
      <h2 class="ei__title">One platform. Different operational realities.</h2>
      <p class="ei__sub">The same SVARA Intelligence Loop adapts to each industry's unique challenges — from the factory floor to the hospital ward to the mine site — without changing its core architecture.</p>
    </div>

    <div class="ei__pipe">
      <div class="ei__rail" :style="{ '--p': (active + 1) / TRANSFORM_STORY.length }" />
      <div v-for="(s, i) in TRANSFORM_STORY" :key="s" class="ei__stage" :class="{ 'is-on': active >= i - 0.4, 'is-last': i === TRANSFORM_STORY.length - 1 }">
...
.ei__title { margin: 0 0 var(--space-5); font-size: clamp(34px, 4.4vw, 56px); font-weight: var(--weight-bold); line-height: 1.1; letter-spacing: -0.02em; color: var(--color-ink-strong); }
.ei__sub { margin: 0; font-size: var(--type-body); line-height: var(--leading-body); color: var(--color-text-secondary); }
...
.ei__name { font-size: clamp(13px, 1.3vw, 17px); font-weight: var(--weight-semibold); color: var(--color-ink-strong); letter-spacing: -0.01em; max-width: 14ch; text-align: center; }
