...
const PRODUCT_ROUTE_KIND: Record<string, string> = {
  "vision-ai": "vision",
  "drone-ai": "drone",
  "edge-ai": "edge",
  "ai-agents": "agents",
  "digital-twin": "twin",
  "ai-os": "aios",
  "business-cloud": "cloud",
  "command-center": "aios",
  "growth-intelligence": "growth",
}

const hudProduct = computed<OsProduct | null>(() => {
  const slug = Array.isArray(route.params.slug) ? route.params.slug[0] : route.params.slug
  const kind = slug ? PRODUCT_ROUTE_KIND[slug] : undefined
  return SVARA_OS.find((p) => p.id === kind) ?? null
})
...
function kpiText(k: OsProduct["kpis"][number]): string {
  return (k.prefix ?? "") + fmt(k.value) + (k.suffix ?? "")
}

const hudMetrics = computed(() => hudProduct.value?.kpis.slice(0, 4).map((k) => ({ label: k.label, value: kpiText(k) })) ?? [])
...
<template>
  <article class="knowledge-product">
    <!-- 00 — Live Product System -->
    <section v-if="hudProduct" class="kp-hud products-page">
      <div class="kp-hud__grid">
        ...
        <div class="kp-hud__window">
          <HUDWindow
            :title="hudProduct.name"
            :system-id="hudProduct.n"
            :code="hudProduct.code"
            :status="hudProduct.status"
            :metrics="hudMetrics"
            :kind="hudProduct.id"
          >
            <WorldViz :kind="hudProduct.id" />
          </HUDWindow>
        </div>
      </div>
    </section>
...
.kp-hud {
  padding: clamp(7rem, 12vw, 11rem) clamp(1.25rem, 5vw, 5rem) clamp(4.5rem, 8vw, 7rem);
  background:
...
.kp-hud__chips span {
  ...
  background: rgba(89, 227, 255, 0.07);
