...
const DARK_FOOTER = new Set(['/', '/investors', '/architecture', '/technology', '/blog', '/divisions', '/contact', '/careers'])
...
//
// About is FILTERED OUT of it, because the Company column below already carries
// that link and the footer listed it twice. Filtered rather than removed from
// primaryNav: that list is the Global Menu's too, where About belongs.
const exploreNav = computed(() => primaryNav.filter(l => l.to !== '/about'))
    ...
    <div class="gf__inner">
      ...
      <div class="gf__nav">
        <nav class="gf__col" aria-label="Explore">
          ...
          <ul class="gf__list">
            <li v-for="l in exploreNav" :key="l.to"><NuxtLink :to="l.to" class="gf__link gf__link--nav">{{ l.label }}</NuxtLink></li>
          </ul>
        </nav>
