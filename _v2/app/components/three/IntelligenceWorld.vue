...
//
// This component owns the 3D world, geometry, lighting, depth, and cursor
// interaction. It does not own product or industry naming, copy, or other
...
const { globalProgress, activeProductIndex, activeIndustryIndex, typographySolid, typographyFontSvh, cameraEnter } =
...
onMounted(() => {
  ...
  // Feature detect: if WebGL is unavailable / disabled / driver-blocked, fail
  // fast and mark the world "ready" so PageLoader doesn't trap the visitor
  // behind the overlay. The scene caption DOM remains fully readable — the
  // experience degrades to a typographic narrative instead of an empty void.
  ...
  const productCenters = Array.from({ length: PRODUCT_COUNT }, (_unused, idx) => {
    const angle = (idx / PRODUCT_COUNT) * Math.PI * 2 - Math.PI / 2;
    return {
  ...
  const industryCenters = Array.from({ length: INDUSTRY_COUNT }, (_unused, idx) => ({
    x: (idx - (INDUSTRY_COUNT - 1) / 2) * INDUSTRY_SPACING,
    y: 0,
    z: -1.5,
  }));
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const productIdx = Math.floor(random() * PRODUCT_COUNT);
    ...
    const industryIdx = Math.floor(random() * INDUSTRY_COUNT);
  ...
  const render = () => {
    ...
    const activeFloat = productsSweep * PRODUCT_COUNT;
    const newActiveProductIndex = Math.min(PRODUCT_COUNT - 1, Math.floor(activeFloat));
    if (productsLayout > 0 && activeProductIndex.value !== newActiveProductIndex) {
      activeProductIndex.value = newActiveProductIndex;
    } else if (productsLayout <= 0 && activeProductIndex.value !== null) {
      activeProductIndex.value = null;
    }
    const activeIndustryFloat = industriesSweep * INDUSTRY_COUNT;
    const newActiveIndustryIndex = Math.min(INDUSTRY_COUNT - 1, Math.floor(activeIndustryFloat));
    if (industriesLayout > 0 && activeIndustryIndex.value !== newActiveIndustryIndex) {
      activeIndustryIndex.value = newActiveIndustryIndex;
    } else if (industriesLayout <= 0 && activeIndustryIndex.value !== null) {
      activeIndustryIndex.value = null;
    }
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (i % 43 === 0) {
        ...
        const streamSpan = (INDUSTRY_COUNT - 1) * INDUSTRY_SPACING;
